function  [cost,w1,w2,w3,b1,b2,b3] = train_part1_ss_matrix(weight1,weight2,weight3,bias1,bias2,bias3,X,desired_output)
...
%         end
%     end
del_w1 = del_w1 +  X' * ((desired_output_mod - Y_output) * weight3' .* d_sigmoid(node_b) * weight2' .* d_sigmoid(node_a));
del_b1 = del_b1 + (desired_output_mod - Y_output) * weight3' .* d_sigmoid(node_b) * weight2' .* d_sigmoid(node_a);
