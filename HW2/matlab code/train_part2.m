function  [cost,w1,w2,w3,b1,b2,b3] = train_part2(weight1,weight2,weight3,bias1,bias2,bias3,X,desired_output)
...
desired_output_mod=desired_output;

cost = 0.5 * ((desired_output_mod - Y_output).^2);
%% back propagation
mu=0.03;
...
%         end
%      end
del_w2 = del_w2 +  (Y_node_a)' *((desired_output_mod - Y_output) * weight3' .* d_sigmoid(node_b)) ;
del_b2 = del_b2 + (Y_output - desired_output_mod) * weight3' .* d_sigmoid(node_b);        
...
%                         * X(1,p) * weight3(k,m) * weight2(j,k) * d_sigmoid(node_b(1,k));
%                     del_b1(1,j) = del_b1(1,j) + (desired_output_mod(1,m) - Y_output(1,m)) * d_hyperbolic_tangent(node_a(1,j)) * weight3(k,m) ...
%                      * weight2(j,k) * d_sigmoid(node_b(1,k));
%                 end
%             end
%         end
%     end
del_w1 = del_w1 +  X' * ((desired_output_mod - Y_output) * weight3' .* d_sigmoid(node_b) * weight2' .* d_hyperbolic_tangent(node_a));
del_b1 = del_b1 + (desired_output_mod - Y_output) * weight3' .* d_sigmoid(node_b) * weight2' .* d_hyperbolic_tangent(node_a);
