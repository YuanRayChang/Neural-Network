function  [cost,w1,w2,w3,b1,b2,b3] = train_part1_ss(weight1,weight2,weight3,bias1,bias2,bias3,X,desired_output)
...
%                      * weight2(j,k) * d_hyperbolic_tangent(node_b(1,k));
                    del_w1(p,j) = del_w1(p,j) + ( desired_output_mod(1,m) - Y_output(1,m)) * sigmoid(node_a(1,j)) ...
                        * X(1,p) * weight3(k,m) * weight2(j,k) * d_sigmoid(node_b(1,k));
                    del_b1(1,j) = del_b1(1,j) + (desired_output_mod(1,m) - Y_output(1,m)) * sigmoid(node_a(1,j)) * weight3(k,m) ...
